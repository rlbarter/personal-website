---
title: "A guide to the tidyverse (part 1)"
author: "Rebecca Barter"
output:
  blogdown::html_page:
    toc: true
categories: [R, tidyverse, dplyr, ggplot2, pipes]
date: 2019-08-02T22:13:14-05:00
type: "post" 
description: "."
---
...
Most people who learned R long before the tidyverse came to be have at some point in the past couple of years felt at least a nibble of pressure to get aboard the tidyverse train. Sadly a fact of human nature is that once you're comfortable doing something a certain way, it's hard to find the motivation to learn a different way of doing something you already know how to do. As someone first learnt R 10 years ago - long before the tidyverse - I've been there. Five years ago, I was pushed to stick my little toe into the shallow-end of the tidyverse pool by learning ggplot2, and I never went back. From where I float now substantially closer to the deep-end of the pool, I would provide the following arguments you as well as to my past self for why it is a good idea to learn the tidyverse:

- Regardless of whether you think the tidyverse or base R is "better", it is always a good idea to keep up with what is current. It's so easy to get left behind. What you learn today will most likely be out-of-date in a year, but next year's iteration of what is current will almost certainly be built upon today's iteration. If you keep up with things as they change, then each new "hot thing" instead of having to take another big jump in a few years, you just need to take lots of little ones. 

- Code written in the tidyverse style is much easier to read, and is more consistent than base R (e.g. the first argument of almost every tidyverse function is the data frame on which it acts, which allows us to make heavy use of "piping"). Base R, on the other hand, has a somewhat inconsistent mish-mash of function and argument styles.

- The humans that make up the tidyverse community is amazing.

While the "tidyverse" is primarily made up of a set of super useful R packages (*ggplot2*, *dplyr*, *purrr*, *tidyr*, *readr*, *tibble*), it is also a way of thinking about implementing "tidy" data analysis. If you combine tidy thinking with the tidy packages, you will inevitably become a master of tidy analysis.
...
The goal of this post is to summarise the overall goals of the tidyverse, the packages that form the tidyverse (and how they play together) and to provide links to helpful resources for learning them.

For new tidyverse practitioners, I'd recommend focusing on getting a handle on piping `%>%`, *dplyr* and *ggplot2* (part one of this post). Once you feel comfortable with these core aspects of the tidyverse, you can move onto [part two](http://www.rebeccabarter.com/blog/2019-08-02_base_r_to_tidyverse_pt2/) of this two-part series on the tidyverse to learn about the remaining packages. 

It is important to remember that the tidyverse is constantly evolving, and the best place to keep up to date with the evolving tidyverse ecosystem is actually twitter. Mara Averick (@dataandme) and Hadley Wickham (@hadleywickham) are good people to follow. A great resource for learning about the tidyverse in more detail is [R for Data Science](https://r4ds.had.co.nz/) by Garrett Grolemund and Hadley Wickham.
...
Now that you've loaded the tidyverse and our data, you're are ready to learn about our first tidy analysis tool: the pipe.
...
I always read the pipe symbol, `%>%`, in my head as *"and then"*. Consider the following code. Can you figure out what the following code will produce (even if you've never seen the `filter` and `select` dplyr functions before - I'll formally introduce these later)?
...
If you were determined not to use piping, but we wanted to do many manipulations, your code would very quickly get messy and difficult to read. In the style of base R, the common way of making code more readable is to define intermediate objects. 
...
To me, the piped version is infinitely more clear, and simultaneously got rid of the need to define any intermediate objects that we would have needed to keep track of while reading the code. Once I got more and more comfortable with piping, I started to find that pretty much *all* of my code uses pipes. 
...
The `filter` and `select` functions that we just introduced are examples of data manipulation functions from the `dplyr` package. 
...
For instance, to keep only the rows that have a recorded population of at least 1 billion, we can use a filtering with a logical statement involving the `pop` variable (again unquoted). 
...
You can specify multiple filter conditions using a comma. Below we filter to rows from 1992 that have a population of at least 100 million in that year.
...
For instance below we add a new variable, `gdp`, to the gapminder data frame. `gdp` is equal to `gdpPercap` multiplied by `pop`, and then look at the first 6 rows of the data frame using the classic `head()` function.  
...
Note that we haven't re-defined the gapminder data frame, so all we have done here is print out the data frame with the additional gdp variable. 

If we wanted to be able to use this gdp variable down the line, we would need to re-define the gapminder data frame so that `gapminder` now corresponds to the version with the `gdp` variable.
...
For instance, we could arrange all rows in the data frame in order of increasing life expectancy.
...
Again, if you wanted your data frame to actually be arranged as specified, you would need to re-define the gapminder data frame. But if you only need it for one quick analysis (e.g. a table), then you don't need to redefine the data frame. 

Below we re-define the gapminder dataset so that the rows are in order of increasing year, and the countries are in alphabetical order within each year (the secondary arrange variable).
...
The code below first filters to the year 2007, and then splits the data frame into groups by continent and adds a row to each group corresponding to the average life expectancy of all of the countries in that group/continent. 
...
The `summarise()` (or `summarize()`) function aggregates across the rows of the data frame. For instance, we can calculate the average life expectancy, as well as the total GDP.
...
The summarise function plays very nicely with the `group_by()` function. For instance, we can group by year and calculate the average life expectancy and total GDP by year.
...
Note that since these are summaries of the data frame itself, we just want to print them out, rather than re-defining the `gapminder` data frame to be equal to these summaries. And since we aren't using them for anything other than to look at, we don't need to define them as a variable (since we won't be using it again later)


## Advanced dplyr

Other dplyr functions that are incredibly useful, but I don't go into in this post include
...
The first tidyverse package I ever learnt was ggplot2 (the more popular older sibling of the non-existent ggplot1). Ggplot2 is the data visualization package that Hadley Wickham based on a set of principles outlines in his *layered grammar of graphics* (inspired by Wilkinson's original *grammar of graphics*). The basic idea is that a ggplot graphic is a layer of mappings from variables in the data frame to geometric objects (circles, lines, etc) called `geom`s. The properties (position, size, color) of the geometric objects based on the variables are specified by the aesthetic (`aes()`) function.

The base layer of any ggplot graph is the empty ggplot layer which defines the data frame that the plot will be based on. We haven't told ggplot what type of geometric object(s) we want yet, nor how the variables should be mapped to the geometric objects, so we just have a blank plot.
...
Since we now know about pipes, we could pipe in our data that we want to plot. Piping makes it easy to do intermediate manipulations that you don't necessarily want to save in the data frame itself, such as only plotting one year's worth of data
...
Layers are added to ggplot objects using `+`, instead of `%>%`, since we are not explicitly piping the LHS into each subsequent layer (we are actually adding a layer on top). The error messages have recently been improved to warn you if you are accidentally using a pipe `%>%` to add layers to ggplot objects (which, once you start piping everything into everything, becomes an easy mistake to make).

The most common geom layer is `geom_point`. Inside `geom_point()`, we specify the aesthetic mappings from the variables to the geometric objects that we want: the `x` position should be `gdpPercap` and the `y` position should be `lifeExp`.
...
Note that since the aesthetics for `geom_point()` and `geom_smooth()` are the same, we might want to just specify global aesthetics in the `ggplot()` function, rather than layer-specific aesthetics.
...
We could also combine a points geom later with a line geom layer. But line plots work well for plotting time series, so we might instead want to plot the average life expectancy over time.

Here we can do some clever combinations of dplyr manipulations with ggplot2 by summarising life expectancy by year and piping the results into a ggplot without having to define any intermediate variables.
...
If we wanted to have one line for each continent, we would need to also first group by `continent` when we calculate our summary. 
...
However when we try to plot a line, we get a weird zig-zag pattern. 
...
This happens because we have multiple average life expectancy values for each year, but we haven't specified which ones go together. To fix this plot, we need to specify how the rows are grouped together by specifying the `group = continent` argument in the `aes()` function of the `geom_line()` layer. 
...
So far we have only specified the x- and y-position aesthetic mappings from the data to the geom objects. But we can also specify other types of aesthetic mappings, such as using a variable to specify the color of the points. 

If you want all of the points to be the same color, you can specify a global point color argument (that lies outside the `aes()` function).
...
However, if you wanted to use a variable from the data frame to define the color (or any other aesthetic feature) of the geoms, **you will need to include it inside the `aes()` function**. 
...
These colours are atrocious though, so don't use these. There are lots of types of scales that you can use for every type of aesthetic mapping (including x- and y-positions), and typically scales are specific to whether your variable using in the aesthetic mapping is discrete or continuous.
...
For our line plot example above where we plotted an average life expectancy time line for each continent, instead of specifying a `group` argument, we could instead specify a `colour` argument to be `continent`. This will will automatically group and color by `continent`.
...
So far, we have only seen scatterplots (points) and line plots, however, there are many other geoms we could add, including:
...
Boxplots are automatically grouped by the x-aesthetic provided (e.g. continent in the plot below). To color boxplots, use the `fill` argument instead of the `color` argument.
...
In [part two](http://www.rebeccabarter.com/blog/2019-08-02_base_r_to_tidyverse_pt2/) of this post on the tidyverse, you will see some ggplot2 code (under the guide of learning about factors) that makes this plot:
